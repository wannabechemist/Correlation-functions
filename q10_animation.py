...
ymax=1 #this is the value of q10 functions or the size of the y axis
Q=1.2
epsilon=1
c0=0.425
#c1=0.005
...
def q10_p0(r,a,c,pi,Q,k0,w,zp):
    s=zp*zp*4*pi*Lb*(rhop1+rhom1*w*w)
    return -(a*c/(4*pi*r*Q*(k0-s)))*(np.exp(-np.sqrt(k0)*r)-
             np.exp(-np.sqrt(s)*r))-(a/(4*pi*r))*np.exp(-np.sqrt(s)*r)
def q10_m0(r,a,c,pi,Q,k0,w,zp):
    s=zp*zp*4*pi*Lb*(rhop1+rhom1*w*w)
...
q_p0,=ax.plot([],[],"r",linewidth=2,label="$q^{10}_{+\,\,\!\!0\!\!}$")
q_m0,=ax.plot([],[],"b",linewidth=2,label="$q^{10}_{-\,\,\!\!0\!\!}$")
q_pp,=ax.plot(r,q10_pp,"r--",linewidth=2,label="$q^{10}_{+\!\!+\!\!}$")
q_pm,=ax.plot(r,q10_pm,"b--",linewidth=2,label="$q^{10}_{+\!\!-\!\!}$")
ax.set_xlabel("r / A",fontsize=14)
ax.set_ylabel("$q^{10}_{+\,\,\!\!0\!\!}$ , $q^{10}_{-\,\,\!\!0\!\!}$ , $q^{10}_{+\!\!+\!\!}$ , $q^{10}_{+\!\!-\!\!}$",fontsize=14)
title=ax.set_title("GRAPH")
labels=["label1","label2"]
ax.legend(loc="upper right",frameon=False,labelspacing=0.4,fontsize=12)
def animate(i):
    r=np.arange(0.1,rmax,0.1)
    y=q10_p0(r,a,c,pi,Q,k0,0.1*i+1,zp)
    z=q10_m0(r,a,c,pi,Q,k0,0.1*i+1,zp)
...
fig.tight_layout()
ani.save("C://Users/tibor mlakar/Desktop/slike magistrska/q10_ANIMACIJE/q10_2b.gif",fps=2)
